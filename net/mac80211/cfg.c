				  ...
				  struct vif_params *params)
{
	...
	if (params->frame_padding_enabled >= 0)
		sdata->frame_padding_enabled = !!params->frame_padding_enabled;

	if (params->frame_padding_size >= 0) {
		u32 size = params->frame_padding_size;

		if (size > IEEE80211_MAX_DATA_LEN)
			size = IEEE80211_MAX_DATA_LEN;
		sdata->frame_padding_size = size;
	}
